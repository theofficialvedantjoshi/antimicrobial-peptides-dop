...
class Pipeline:
    ...
    def _estimate_hyperparameters(
        self, X: np.ndarray, y: np.ndarray, scoring: list = ["accuracy", "f1_macro"]
    ...
    def _save_hyperparameters(self, model: str = "mlknn") -> None:
    ...
    def _save_predictions(
        ...
        with pd.ExcelWriter(os.path.join(ROOT, model, "predictions.xlsx")) as writer:
    ...
    def run(self) -> None:
        if not self._load_hyperparameters():
            ...
            for feature in FEATURES:
                for i, (X_train, y_train) in enumerate(self.data[feature]["train"]):
                    ...
                    params, _ = self._estimate_hyperparameters(X_train, y_train)
                    print(params)
        ...
        for feature in FEATURES:
            for i in range(len(self.data[feature]["train"])):
                ...
                print(f"Running MLkNN for {feature} dataset {i}...")
                predictions = self._mlknn(
                    X_train,
                    y_train,
                    X_test,
                    k=self.hyperparams[feature][i]["k"],
                    s=self.hyperparams[feature][i]["s"],
                )
        ...
        self._save_predictions(final_predictions)
